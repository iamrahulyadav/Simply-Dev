...
public class BalanceHelper {
	...
	public List<NormalCall> getAllEntries() {
		...
		// 1-DATE 2-COST 3-DURATION 4-NUMBER 5-BALANCE 6 - TEXT
		if (cursor.moveToFirst()) {
			do {
				entry = new NormalCall();
				entry.date = (Long.parseLong(cursor.getString(1)));
				entry.callCost = Float.parseFloat(cursor.getString(2));
				
				entry.callDuration = (Integer.parseInt(cursor.getString(3)));
				entry.lastNumber = cursor.getString(4);
				entry.bal = Float.parseFloat(cursor.getString(5));
				entry.message = cursor.getString(6);
				// Add book to entry
				entries.add(entry);
...
/*	CREATE TABLE CALL ( "
	...
	public List<NormalCall> getEntriesFromDate(long time,Context ctx) {
		List<NormalCall> entries = new LinkedList<NormalCall>();
		float total_call_cost = (float) 0.0;
		int total_call_duration = 0;
		// 1. build the query
		String query = "SELECT  * FROM " + "CALL" + " WHERE date >= " + time;
...
//				SQLiteDatabase.OPEN_READONLY);
		...
		// SQLiteDatabase db = this.getWritableDatabase();
		Cursor cursor = myDataBase.rawQuery(query, null);
		SharedPreferences mSharedPreferences = ctx.getSharedPreferences(
