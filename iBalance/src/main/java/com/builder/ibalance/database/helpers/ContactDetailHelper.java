...
public class ContactDetailHelper
{
    ...
    {
        Cursor c = mSqlDB.query(IbalanceContract.ContactDetailEntry.TABLE_NAME, null, null, null, null, null, null);
        return c;
    }
    ...
    public ContactDetailModel getPopUpDetails(String phNumber)
    {
        ...
        if (c.moveToFirst())
        {
            ...
            m = new ContactDetailModel(c.getString(c.getColumnIndex(IbalanceContract.ContactDetailEntry.COLUMN_NAME_NAME)), c.getString(c.getColumnIndex(IbalanceContract.ContactDetailEntry.COLUMN_NAME_CARRIER)), c.getString(c.getColumnIndex(IbalanceContract.ContactDetailEntry.COLUMN_NAME_CIRCLE)), c.getString(c.getColumnIndex(IbalanceContract.ContactDetailEntry.COLUMN_NAME_IMAGE_URI)), total_cost);
            return m;
        }
        ...
        if (phNumber.length() >= 10)
        {
            try
            {
                carrier_circle = (new MappingHelper()).
                        getMapping(Integer.parseInt(phNumber.substring(phNumber.length() - 10, phNumber.length() - 6)));
        ...
        if (phNumber != null)
        {
            ...
            try
            {
                if (c.moveToFirst())
                {
                    ...
                    m = new ContactDetailModel(c.getString(c.getColumnIndex(IbalanceContract.ContactDetailEntry.COLUMN_NAME_NAME)), c.getString(c.getColumnIndex(IbalanceContract.ContactDetailEntry.COLUMN_NAME_CARRIER)), c.getString(c.getColumnIndex(IbalanceContract.ContactDetailEntry.COLUMN_NAME_CIRCLE)), c.getString(c.getColumnIndex(IbalanceContract.ContactDetailEntry.COLUMN_NAME_IMAGE_URI)), 0.0f);
                    return m;
                }
    ...
    public int getOutDurationLocalSameCarrier(String sim_circle, String sim_carrier)
    {
        ...
        Cursor c = mSqlDB.rawQuery("SELECT SUM(" + IbalanceContract.ContactDetailEntry.COLUMN_NAME_OUT_DURATION + ") FROM " +
                IbalanceContract.ContactDetailEntry.TABLE_NAME + " WHERE " +
                IbalanceContract.ContactDetailEntry.COLUMN_NAME_CIRCLE + " = \"" + sim_circle + "\" " + " AND " + IbalanceContract.ContactDetailEntry.COLUMN_NAME_CARRIER + " = \"" + sim_carrier + "\"", null);
        try
        {
            if (c.moveToFirst())
            {
