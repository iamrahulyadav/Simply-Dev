...
public class CircleIndicator extends LinearLayout implements OnPageChangeListener {
    ...
    @Override public void onPageSelected(int position) {
    	//Log.d("TUTORIAL", "circle position = "+position);
    	if(position>=5)
    		{
    		SharedPreferences mSharedPreferences = mContext.getSharedPreferences("USER_DATA", Context.MODE_PRIVATE);
			Editor mEditor = mSharedPreferences.edit();
			mEditor.putBoolean("TUTORIAL", true);
			mEditor.commit();
			mContext.startActivity(new Intent(mContext, Wizard.class));
